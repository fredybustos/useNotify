...
const notify = ({ type, message, options }: NotifyProps) => {
  ...
  ReactDOM.createRoot(container).render(
    <Notifications openNotify notification={notification} container={container} />
  )
...
const Notify = ({ type, message, options }: NotifyProps) => notify({ type, message, options })

const success = (message: string, options?: Options) =>
  notify({ type: 'success', message, options })

const warn = (message: string, options?: Options) => notify({ type: 'warning', message, options })

const error = (message: string, options?: Options) => notify({ type: 'error', message, options })
